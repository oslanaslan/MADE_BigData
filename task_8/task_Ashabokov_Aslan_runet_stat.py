...
res_df = (
    ...
    .withColumn("window", F.from_unixtime(F.split(F.col("value"), "\t").getItem(0), "yyyy-MM-dd HH:mm:ss"))
...
output = spark.sql("""
    select window, count(uid) as view, approx_count_distinct(uid) as unique
    from page_views_tmp
    group by window
    order by window asc
    limit 10
